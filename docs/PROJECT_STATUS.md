...
- Verified implementation baseline SHA: `ffaea9bc9baf38fb7bb256e7dbeeb51133e659cb`
- Status verified against origin/main: `ffaea9bc9baf38fb7bb256e7dbeeb51133e659cb`
...
- PR #75 — Student Entitlement Visibility — Slice 1 merged at `722b66a8bb7bcd7c507cbd6e87c07889f5e22c07`; Student Payments now includes a server-authoritative course-access view derived from active Enrollments, distinguishes permanent versus time-bounded access, shows the bounded payment source reference/purpose, and projects existing Included Evaluation Entitlements as Available, Consumed, or Revoked. No schema, pricing, checkout, provider, wallet, refund, or entitlement-rule behavior changed. Required PR Quality #209, Full UAT Matrix #94, and Security analysis #211 were green before merge; post-merge `main` Quality #211 and Security analysis #213 are green.
- PR #74 — Broader ASSESS Reasonable Adjustments — Slice 1 merged at `ffaea9bc9baf38fb7bb256e7dbeeb51133e659cb`; BETCCO's one revised-work check now has a server-owned revision deadline, staff-authorised grant/revoke/history for deadline extensions, effective-deadline enforcement on resubmission, learner-visible effective deadline without private staff rationale, and CourseReviewer coordination UI. Additive migration `20260925233350_AddEvaluationRevisionDeadlineAdjustments` was introduced. No P/M/D, payment, entitlement, extra-attempt, Retake/Resit, AI, or LEARN coursework-deadline behavior changed. Required PR Quality #212, Full UAT Matrix #96, and Security analysis #214 were green before merge; post-merge `main` Quality #213 and Security analysis #215 are green.
...
- Retake Slice 1 historical foundation — merged through PR #36 in `717ddd5`; new Retake creation/authorization was later retired by PR #63 while historical records remain supported.
...
Done: Main contains the versioned criterion/rule and qualification snapshot foundations, the additive ASSESS academic identity foundation from PR #28 (`UnitDefinition`, `AssessmentDefinition`, `AssessmentScope`, and the nullable `EvaluationRequest` bridge), authenticity declarations, authorised resubmission records, internal-verification sampling, appeals, structured assessment-audit export, formal visual PDF reporting, the historical Retake foundation from PR #36, and Evaluator-specialism routing Slice 1 from PR #38. ASSESS Slices 1–3 are DONE: the canonical academic chain is `QualificationVersion` → `UnitDefinition` → `LearningAimDefinition` → `AssessmentCriterionDefinition`, with `AssessmentDefinition` → `AssessmentScope`; `EvaluationRequest` remains the ASSESS aggregate root. PR #63 later simplified BETCCO into an advisory assignment-review flow with at most one revised-work check and retired creation/authorization of new Pearson-style Retakes; historical Retake records remain readable and can complete through their legacy path. Evaluator assignment requires an active UnitDefinition specialism grant checked server-side, and each new assignment retains the exact grant evidence. The structured audit export now uses the safe v2 contract: raw internal identifiers and sensitive implementation metadata are structurally excluded, academic traceability is preserved, actor-role attribution is corrected, unsupported historical RBAC precision is not fabricated, and deterministic SHA-256 integrity is preserved. The PDF includes assignment/task, submission/evidence, assessor/internal-verifier/lead-internal-verifier/appeal-review, and academic audit/history context; excludes sensitive storage/internal IDs; retains neutral/non-official BTEC wording; and has verified Arabic/English, multipage, and Linux/Windows font-layout compatibility with required CI green. PR #34 separately completed student-specific LEARN coursework deadline extensions without changing `EvaluationRequest` or `ResubmissionAuthorization.DueAtUtc`; LEARN remains separate from ASSESS.

Remaining: Resit policy/workflow remains unresolved on `main`. PR #74 completed the current broader ASSESS Reasonable Adjustments revision-deadline slice for BETCCO's single revised-work check; additional ASSESS accommodations are not implied complete without separate evidence. New Retake creation and Appeal → Retake expansion are not current roadmap behavior after PR #63 retired new Pearson-style Retakes. Canonical LEARN units and AcademicYear/Term/DeliveryPlan Slice 1 are merged through PRs #46 and #45. Production payment/provider validation and production operations remain outstanding.
...
At the 2026-09-26 reconciliation snapshot, PR #75 — Student Entitlement Visibility — Slice 1 is merged at `722b66a8bb7bcd7c507cbd6e87c07889f5e22c07`, and PR #74 — Broader ASSESS Reasonable Adjustments — Slice 1 is merged at `ffaea9bc9baf38fb7bb256e7dbeeb51133e659cb`. Required PR checks were green before both merges; post-merge `main` Quality #213 and Security analysis #215 are green. No OPEN implementation PRs were present at this snapshot, and no ASUS or LENOVO implementation workstream is active.

## Next Actions

- ASUS: PR #75 — Student Entitlement Visibility — Slice 1 is DONE at `722b66a8bb7bcd7c507cbd6e87c07889f5e22c07`. ASUS owns the planned Commerce / Entitlements roadmap, followed by Production Hardening and Final UI/UX Redesign. The immediate next action is a fresh, read-only Commerce / Entitlements gap analysis against current `main`; production provider validation and the unresolved Refund → Course Access business policy must be treated separately, and no refund/access policy change should be implemented without an explicit product decision. ASUS must not take Resit/ASSESS work reserved to LENOVO. AI engine/API/RAG/persistence remains explicitly deferred.
- LENOVO: PR #74 — Broader ASSESS Reasonable Adjustments — Slice 1 is DONE at `ffaea9bc9baf38fb7bb256e7dbeeb51133e659cb`. LENOVO owns the next ASSESS planning lane: Resit policy / workflow. Start with a fresh read-only preflight and policy/design analysis; do not create a Resit implementation branch until the exact BETCCO policy is approved. LENOVO must not edit Commerce / Entitlements, payment/refund/access logic, or ASUS-reserved roadmap work. If later Resit UI requires shared files such as `student-area.tsx`, perform overlap review before editing.
...
- Retake history: PR #36 originally implemented authorised paid assessment Retakes, but PR #63 later retired creation and authorization of new Pearson-style Retakes in favor of the advisory BETCCO review flow. Historical Retake records remain readable and can finish through their legacy path.
...
- Canonical LEARN unit migration: merged through PR #46; ambiguous historical modules still need explicit evidence-backed reconciliation. Resit remains unimplemented. New Retake creation is retired by PR #63. PR #74 completed the current ASSESS revision-deadline reasonable-adjustment slice; any broader accommodation beyond that requires a separately scoped workstream.
...
- PR #30 is merged into `main` at `689b87280ecf27b1d4f825092e8f814066e9ccd4`; versioned academic authoring, canonical aims and criteria, definition mappings, source/publication rules, qualification/rubric compatibility, and the bilingual Admin Academic Catalogue are DONE. Slice 3 subsequently completed AssessmentScope selection and the immutable EvaluationRequest academic snapshot; PR #34 completed LEARN coursework deadline extensions; PR #36 added the historical Retake foundation; PR #38 completed evaluator-specialism routing; PR #45 completed AcademicYear/Term/DeliveryPlan Slice 1; PR #46 completed canonical LEARN unit links; and PR #47 completed Pearson academic catalogue authority and Admin programme planning. PR #63 later retired new Pearson-style Retake creation/authorization in favor of the advisory BETCCO review flow. Resit remains future work.
...
- PR #36 is merged into `main` at `717ddd5b81e2825cdf45dda80bfb4fcac7d7da93`; it established the historical Retake data/workflow foundation with additive migration `20260922182302_AddAssessmentRetakes`, retained approval history, Retake-only scope protection, one-Retake uniqueness, and `RESTRICT` foreign keys. PR #63 later retired creation/authorization of new Pearson-style Retakes while preserving historical Retake readability/completion through the legacy path. ResubmissionAuthorization semantics, including `ResubmissionAuthorization.DueAtUtc`, were not changed; LEARN remains separate from ASSESS. Resit, production migration, production smoke verification, and production payment/provider validation remain outstanding.
...
- Resit policy and workflow remains unresolved. New Pearson-style Retake creation remains retired by PR #63; historical Retake records remain supported only through their legacy path.
