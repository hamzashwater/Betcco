...
- PR #78 — Admin Payout Lifecycle UI Contract — Slice 1 merged at `3d5affe6ac3ca98bda78273f41d9bf455a51c03f`; ASUS added the Admin payout lifecycle UI contract in the existing financial/admin surface without overlapping the LENOVO Resit backend workstream.
- PR #77 — Resit Authorization Foundation — Slice 1 merged at `0bea28b9d99c41345406fdddf416d885076535e1`; CourseReviewer-only Resit eligibility/authorization is now server-owned for completed second-attempt `NotYetAchieved` advisory reviews, with one lifetime authorization per original, active-appeal and historical-Retake/Resit-chain blockers, revoke-before-activation, staff-only rationale, audit evidence, additive `20260926151412_AddEvaluationResitAuthorizations` migration, and PostgreSQL-backed constraint coverage. This slice does not create or activate a Resit `EvaluationRequest`, does not add Resit evidence upload or UI, and does not change Payment/Refund/Entitlements/Course Access. Required PR Quality, Full UAT Matrix, and Security analysis were green before merge; post-merge `main` Application quality and both CodeQL jobs are green.
...
Remaining: Resit Authorization Foundation is complete through PR #77, but Resit activation / new `EvaluationRequest` lifecycle, fresh-evidence submission, evaluator review, result presentation, and the Commerce handoff remain future work. PR #74 completed the current broader ASSESS Reasonable Adjustments revision-deadline slice for BETCCO's single revised-work check; additional ASSESS accommodations are not implied complete without separate evidence. New Retake creation and Appeal → Retake expansion are not current roadmap behavior after PR #63 retired new Pearson-style Retakes. Canonical LEARN units and AcademicYear/Term/DeliveryPlan Slice 1 are merged through PRs #46 and #45. Production payment/provider validation and production operations remain outstanding.
...
- Canonical LEARN unit migration: merged through PR #46; ambiguous historical modules still need explicit evidence-backed reconciliation. Resit Authorization Foundation is merged through PR #77; Resit activation / new-request lifecycle remains unimplemented. New Retake creation is retired by PR #63. PR #74 completed the current ASSESS revision-deadline reasonable-adjustment slice; any broader accommodation beyond that requires a separately scoped workstream.
...
- Resit Authorization Foundation is DONE through PR #77; Resit activation / new `EvaluationRequest` lifecycle remains unresolved. New Pearson-style Retake creation remains retired by PR #63; historical Retake records remain supported only through their legacy path.
